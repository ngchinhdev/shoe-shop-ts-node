...
async function generateMarkup(categoriesData: ICategory[], startIdx: number, container: HTMLDivElement) {
    container.innerHTML = '';
...
function markupRow(cate: ICategory, index: number, startIdx: number) {
    return `<tr>
                <td>
                    ${startIdx + index + 1}
                </td>
...
export default async function initCategories(container: HTMLDivElement, curPage = 0) {
    const categoriesData = await getFullData('categories');
